...
<section class="flex gap-16" id="information">
    ...
    <div class="ml-auto">
	<h2 class="text-xl font-semibold text-slate-700">@lang('pdf_invoice_label')</h2>
	<div class="grid grid-cols-2 gap-x-3">
	    <span class="text-slate-600">@lang('pdf_invoice_number')</span>
	    <span class="text-right">{{ $invoice->invoice_number }}</span>
	    <span class="text-slate-600">@lang('pdf_invoice_date')</span>
	    <span class="text-right">{{ $invoice->formattedInvoiceDate }}</span>
	    <span class="text-slate-600">@lang('pdf_invoice_due_date')</span>
	    <span class='text-right'>{{ $invoice->formattedDueDate }}</span>
	    <span class="text-slate-600">@lang('pdf_invoice_order_reference')</span>
	    <span class='text-right'>{{ $invoice->getFormattedString('{CUSTOM_INVOICE_REFERENCE_ORDER}') }}</span>
	</div>
    </div>
</section>
...
<section class="my-8 break-inside-avoid" id="items">
    @include('app.pdf.invoice.partials.table-tailwind')
</section>
<section class="text-center text-gray-500">
	Gelieve het totaalbedrag voor de vervaldatum over te maken naar LT55 3250 0322 8210 5225 (swift/bic REVOLT21) onder vermelding van het factuurnummer.
</section>
